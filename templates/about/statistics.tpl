...
<table width="100%" class="data">
	...
	{if $statCountDecline}<tr valign="top">
		<td width="20%" >&nbsp;&nbsp;{translate key="manager.statistics.statistics.count.decline"}</td>
	...
	{if $statCountRevise}<tr valign="top">
		<td width="20%" >&nbsp;&nbsp;{translate key="manager.statistics.statistics.count.revise"}</td>
	...
	{if $statDaysPerReview}<tr valign="top">
		<td width="20%" >&nbsp;&nbsp;{translate key="manager.statistics.statistics.daysPerReview"}</td>
	...
	{if $statDaysToPublication}<tr valign="top">
		<td width="20%" >&nbsp;&nbsp;{translate key="manager.statistics.statistics.daysToPublication"}</td>
		<td colspan="2" class="value">{$limitedArticleStatistics.daysToPublication}</td>
	</tr>{/if}
	{if $statRegisteredUsers}<tr valign="top">
		<td width="20%" >{translate key="manager.statistics.statistics.registeredUsers"}</td>
	...
	{if $statRegisteredReaders}<tr valign="top">
		<td width="20%" >{translate key="manager.statistics.statistics.registeredReaders"}</td>
	...
	{if $currentJournal->getSetting('publishingMode') == $smarty.const.PUBLISHING_MODE_SUBSCRIPTION && $statSubscriptions}
		<tr valign="top">
			<td colspan="3" >{translate key="manager.statistics.statistics.subscriptions"}</td>
		</tr>
